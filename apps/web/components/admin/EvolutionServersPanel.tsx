...
function HealthBadge({ isHealthy }: { isHealthy: boolean }) {
  return (
    <span className={cn(
...
export function EvolutionServersPanel() {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold flex items-center gap-2">
            <Server className="h-5 w-5 text-primary" /> Servidores Evolution
          </h2>
          <p className="text-sm text-muted-foreground mt-0.5">
            Gerencie os servidores Evolution API para distribuição de instâncias WhatsApp.
          </p>
        </div>
        {!showForm && !editingId && (
          <button
            onClick={() => { setShowForm(true); setEditingId(null); setForm(EMPTY_FORM) }}
            className="inline-flex shrink-0 items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-primary text-primary-foreground rounded-md hover:bg-primary/90"
          >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <HealthBadge isHealthy={server.isHealthy} />
                  </div>
