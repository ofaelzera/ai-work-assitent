...
export default function EditFileModal({ file, onClose }: Props) {
  ...
  const { data: shares = [] } = useQuery<ShareEntry[]>({
    ...
    queryFn: () => apiFetch(`/storage/files/${file.id}/shares`),
  ...
  const saveMeta = useMutation({
    mutationFn: () => apiFetch(`/storage/files/${file.id}`, {
      method: 'PATCH',
      body: JSON.stringify({ filename: filename.trim(), visibility }),
    }),
    onSuccess: () => {
  ...
  const addShare = useMutation({
    mutationFn: (userId: string) => apiFetch(`/storage/files/${file.id}/shares`, {
  ...
  const removeShare = useMutation({
    mutationFn: (userId: string) => apiFetch(`/storage/files/${file.id}/shares/${userId}`, {
      method: 'DELETE',
    }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['file-shares', file.id] }),
    onError: () => toast.error('Erro ao remover acesso'),
  })

  const sharedUserIds = new Set(shares.map(s => s.userId))
  const availableToShare = users.filter(u => u.id !== file.uploadedBy && !sharedUserIds.has(u.id))
      ...
      <div className="modal-surface rounded-xl shadow-2xl w-full max-w-md flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-sm">Editar arquivo</h2>
          </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {([
                { v: 'PRIVATE' as Visibility, icon: Lock, label: 'Privado', desc: 'Só você', color: 'text-amber-600' },
                { v: 'SHARED'  as Visibility, icon: Users, label: 'Compartilhado', desc: 'Pessoas específicas', color: 'text-blue-600' },
                { v: 'PUBLIC'  as Visibility, icon: Globe, label: 'Público', desc: 'Todos veem', color: 'text-emerald-600' },
          ...
          {visibility === 'SHARED' && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium">Compartilhado com</label>
                <button onClick={() => setPickUserOpen(v => !v)}
                ...
                <div className="rounded-lg border bg-muted/20 max-h-40 overflow-y-auto">
                  {availableToShare.length === 0 ? (
                    ...
                    availableToShare.map(u => (
                      <button key={u.id} onClick={() => addShare.mutate(u.id)}
              ...
              {shares.length === 0 ? (
                <p className="text-xs text-muted-foreground italic">Ninguém compartilhado ainda.</p>
              ) : (
                <div className="rounded-lg border divide-y">
                  {shares.map(s => (
                    <div key={s.id} className="flex items-center gap-2 px-3 py-2 group">
                      ...
                      <button onClick={() => removeShare.mutate(s.userId)}
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t shrink-0">
          ...
          <button
            onClick={() => filename.trim() && saveMeta.mutate()}
            disabled={!filename.trim() || saveMeta.isPending}
